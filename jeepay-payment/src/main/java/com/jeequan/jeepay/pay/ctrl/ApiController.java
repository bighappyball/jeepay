...
public abstract class ApiController extends AbstractCtrl {
     ...
     * 获取请求参数并转换为对象，通用验证
     **/
    protected <T extends AbstractRQ> T getRQ(Object o, Class<T> cls) {
        T bizRQ = BeanUtils.instantiateClass(cls);
        BeanUtils.copyProperties(o,bizRQ);
     ...
     * 获取请求参数并转换为对象，商户通用验证
     **/
    protected <T extends AbstractRQ> T getRQByWithMchSign(Object cls, Class<T> tClass) {

        //获取请求RQ, and 通用验证
        T bizRQ = getRQ(cls, tClass);
